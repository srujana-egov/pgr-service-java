...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { PgrServiceInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw PgrServiceInvalidDataException("`$name` is not set")
            is JsonNull -> throw PgrServiceInvalidDataException("`$name` is null")
            else -> throw PgrServiceInvalidDataException("`$name` is invalid, received $this")
        }
