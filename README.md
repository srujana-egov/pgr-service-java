...
PgrServiceClient client = PgrServiceOkHttpClient.fromEnv();

ServiceWrapper params = ServiceWrapper.builder().build();
ServiceResponse serviceResponse = client.citizenService().create(params);
```
...
| --------- | -------------------- | ---------------------- | -------- | ------------------------- |
| `apiKey`  | `pgrservice.apiKey`  | `PGR_SERVICE_API_KEY`  | true     | -                         |
| `baseUrl` | `pgrservice.baseUrl` | `PGR_SERVICE_BASE_URL` | true     | `"http://localhost:8083"` |
...
To send a request to the Pgr Service API, build an instance of some `Params` class and pass it to the corresponding client method. When the response is received, it will be deserialized into an instance of a Java class.

For example, `client.citizenService().create(...)` should be called with an instance of `CitizenServiceCreateParams`, and it will return an instance of `ServiceResponse`.
...
ServiceWrapper params = ServiceWrapper.builder().build();
CompletableFuture<ServiceResponse> serviceResponse = client.async().citizenService().create(params);
```
...
// Configures using the `pgrservice.apiKey` and `pgrservice.baseUrl` system properties
// Or configures using the `PGR_SERVICE_API_KEY` and `PGR_SERVICE_BASE_URL` environment variables
PgrServiceClientAsync client = PgrServiceOkHttpClientAsync.fromEnv();

ServiceWrapper params = ServiceWrapper.builder().build();
CompletableFuture<ServiceResponse> serviceResponse = client.citizenService().create(params);
```
...
Enable logging by setting the `PGR_SERVICE_LOG` environment variable to `info`:

```sh
export PGR_SERVICE_LOG=info
```

Or to `debug` for more verbose logging:

```sh
export PGR_SERVICE_LOG=debug
```

## ProGuard and R8

Although the SDK uses reflection, it is still usable with [ProGuard](https://github.com/Guardsquare/proguard) and [R8](https://developer.android.com/topic/performance/app-optimization/enable-app-optimization) because `pgr-service-java-core` is published with a [configuration file](pgr-service-java-core/src/main/resources/META-INF/proguard/pgr-service-java-core.pro) containing [keep rules](https://www.guardsquare.com/manual/configuration/usage).
